...
const BookingDialog = ({ bookDialogIsOpen, closeBookDialog, id }) => {
    ...
    const handleBookTicket = async (e) => {
        e.preventDefault();
        await dp(bookTicket({ name, email, id, eq }));
    };
    return (
        <Dialog
                ...
                <DialogContent style={{ padding: "30px", minWidth: "40%" }}>
                    ...
                    <form onSubmit={handleBookTicket}>
                        ...
                        <Button fullWidth variant="contained" type="submit" disabled={loadingBookTicket == FS.FETCHING}>
                            {loadingBookTicket == FS.FETCHING ? <BeatLoader color="#fff" size={8} /> : "Book now"}
                        </Button>
                    </form>
                </DialogContent>
            </CustomClass>
