...
const VIP = () => {
    ...
    const [point, setPoint] = useState(vipAccount.point);
    ...
    const handleChangePoint = (e) => {
        if (patternRange.test(e.target.value)) {
            if (e.target.value > vipAccount.point) {
                eq(`Number of point max is ${vipAccount.point} only!!!`, WARNING_TOP_CENTER);
                setPoint("");
                return;
            }
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} md={2}>
                        <Box className="middle">
                            ...
                            <Typography style={{ fontSize: "25px" }}>{vipAccount.point}</Typography>
                        </Box>
                    </Grid>
                    <Grid item md={1}></Grid>
                    <Grid item xs={12} md={4}>
                        ...
                        <Button variant="contained" fullWidth onClick={exchangeForVoucher} disabled={loading}>
                            {loading ? <BeatLoader color="#fff" size={8} /> : "Exchange for voucher"}
                        </Button>
                    </Grid>
                </Grid>
            </Paper>
